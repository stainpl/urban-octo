...
import {
  FiMenu, FiX, FiHome, FiFileText, FiUser, FiUserPlus, FiLogIn, FiLogOut, FiSearch, FiMessageSquare
} from 'react-icons/fi';
    ...
    <header className="w-full bg-white dark:bg-gray-900 border-b">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6">
          <Link href="/" className="link-gradient flex items-center gap-2 text-sm"><FiHome/> Home</Link>
          <Link href="/my-blog" className="link-gradient flex items-center gap-2 text-sm"><FiFileText/> Blog</Link>
          <Link href="/about" className="link-gradient flex items-center gap-2 text-sm"><FiUser/> About</Link>
          <button aria-label="search" className="p-1 rounded hover:bg-gray-500"><FiSearch/></button>
          <button aria-label="contact" className="p-1 rounded hover:bg-gray-500"><FiMessageSquare/></button>

          {!user ? (
            <div className="flex items-center gap-2">
              <button
                onClick={() => setShowLoginModal(true)}
                className="flex items-center gap-2 text-sm px-3 py-1 border rounded hover:bg-gray-500"
                title="Login / Sign up"
              >
            ...
            <div className="flex items-center gap-3">
              ...
              {/* If admin, quick link to admin area */}
              {user.role === 'admin' && (
                <Link href="/admin" className="text-sm px-3 py-1 border rounded hover:bg-gray-50">Admin</Link>
              )}
        ...
        <div className="md:hidden flex items-center">
          {!user ? (
            <button onClick={() => setShowLoginModal(true)} className="mr-2 p-2 rounded border text-sm">
              <FiLogIn />
            </button>
          ) : (
      ...
      {/* Mobile menu panel */}
      {open && (
        <div className="md:hidden bg-gray-450 border-t">
            ...
            <div className="border-t pt-3">
              {!user ? (
                <div className="flex flex-col gap-2">
                  ...
                  <Link href="/signup" onClick={() => setOpen(false)} className="py-2 rounded border flex items-center justify-center gap-2">
                    <FiUserPlus /> Sign up
                  </Link>
                </div>
              ) : (
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => { handleLogout(); setOpen(false); }} className="p-2 rounded border">
                    <FiLogOut />
                  </button>
                </div>
              )}
